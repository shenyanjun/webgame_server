...
----------------------------------玩家的选号
require("spec_lottery.slottery_number")

Slottery_numbers = oo.class(nil, "Slottery_numbers")

local limits = 5000			--下注范围   0 ~ 5000
local lvls	 = 4			--获奖等级

function Slottery_numbers:__init(data)
	if data then
		self:load(data)
	else
		self.numbers 	= {}
...
function Slottery_numbers:get_total_count()
	local total_count = 0
...
function Slottery_numbers:choice_number(char_id,number)
	if number >= 0 and number <= limits then
		...
		if self.numbers[s_number] then
			...
			self.numbers[s_number] = Slottery_number()
			self.numbers[s_number]:choice_number(char_id)
		end
	end
end
...
function Slottery_numbers:draw_lottery()
...
-----------------------------------抽取鼓励奖
	...
	if pkt then
		for i = 10, 1,-1 do
			...
			winners[4][number] = {}
			winners_name[4][number] = {}

			if location ~= 0 then
				winners[4][number] = self.numbers[number]:get_player_info()
				winners_name[4][number] = self.numbers[number]:get_player_name_info()
			end
	...
	if pkt then
		for i = 5, 1, -1 do
			...
			winners[3][number] = {}
			winners_name[3][number] = {}

			if location ~= 0 then
				winners[3][number] = self.numbers[number]:get_player_info()
				winners_name[3][number] = self.numbers[number]:get_player_name_info()
			end
	...
	if pkt then
		for i = 2, 1, -1 do
			...
			winners[2][number] = {}
			winners_name[2][number] = {}

			if location ~= 0 then
				winners[2][number] = self.numbers[number]:get_player_info()
				winners_name[2][number] = self.numbers[number]:get_player_name_info()
			end
	...
	if pkt then
		for i = 1, 1 do
			...
			winners[1][number] = {}
			winners_name[1][number] = {}

			if location ~= 0 then
				winners[1][number] = self.numbers[number]:get_player_info()
				winners_name[1][number] = self.numbers[number]:get_player_name_info()
			end
...
-------------------------------------------------存盘
function Slottery_numbers:spec_serialize_to_db()
	return self.numbers
end

function Slottery_numbers:load(db_data)
	self.numbers	= {}
	for k , v in pairs(db_data) do
		self.numbers[k] = Slottery_number(v)
	end
end
