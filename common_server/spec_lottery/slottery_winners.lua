...
Slottery_winners = oo.class(nil, "Slottery_winners")

local delay_time = 60 * 5
local lvls = 4			--下注范围   0 ~ 5000
local winners_email = {}
function Slottery_winners:__init( data )
	if data then
...
function Slottery_winners:notice_lottery()
--构造字符串并广播
	local pkt = {}
	winners_email = {}
...
function Slottery_winners:structure_all_email()
	local f = function()
...
---------构造中奖玩家邮件
function Slottery_winners:structure_email(char_id,bonus,lvl)
	local submit_time = f_get_sunday() + 8 * 24 * 3600 + 1
	local day_table	  = os.date("*t" , submit_time)
	local draw_time = f_get_sunday() + 24 * 3600 + 1
	local draw_table  = os.date("*t" , draw_time)
...
-------------------------------------------------存盘
function Slottery_winners:spec_serialize_to_db()
	return self
end

function Slottery_winners:load(db_data)
...
-------------------------------------------------存盘
function Slottery_winners:spec_serialize_to_map()
	return self.winners
end