...
module("mall.mall_loader", package.seeall)
...
function init()
	local filename = CONFIG_DIR .. "xml/mall/mall.xml"
	--[[if _DEBUG then
		filename = Server_path .. "map/config/xml/mall_debug.xml"
	end]]
		...
		if xml_tree.tag == "Catalog" then
			...
			for i, xml_node in pairs(xml_tree) do
				if type(xml_node.tag) == "string" and xml_node.tag == "item" then
					...
					--t_node.item_list[item_count].id = 
...
function handle_page(t_node)
	for k,v in pairs(t_node.item_list) do
		...
		list.price = {[GOLD]=v.price,[GIFT_GOLD]=v.price,[JADE]=v.price,[GIFT_JADE]=v.price}
		list.currency = v.currency
