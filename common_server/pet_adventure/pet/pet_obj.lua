...
D_pet_obj = oo.class(nil,"D_pet_obj")

function D_pet_obj:__init(pet_id,owner_id)
...
function D_pet_obj:get_element_de_t()
	return 0
end

function D_pet_obj:get_d_critical_ef_t()
	return 0
end

function D_pet_obj:get_init_strengh()
	return self.init_strengh
end

function D_pet_obj:set_init_strengh(strengh)
	self.init_strengh = strengh
end

function D_pet_obj:get_init_intelligence()
	return self.init_intelligence
end

function D_pet_obj:set_init_intelligence(intelligence)
	self.init_intelligence = intelligence
end

function D_pet_obj:get_init_stemina()
	return self.init_stemina
end

function D_pet_obj:set_init_stemina(stemina)
	self.init_stemina = stemina
end


function D_pet_obj:get_init_dexterity()
	return self.init_dexterity
end

function D_pet_obj:set_init_dexterity(dexterity)
	self.init_dexterity = dexterity
end

function D_pet_obj:get_combat_status()
	return self.combat_status
end

function D_pet_obj:set_combat_status(combat_status)
	self.combat_status = combat_status
end

function D_pet_obj:get_base_skill_id()
	return pet_expr.get_base_skill(self.occ)
end

function D_pet_obj:get_occ_level()
	return pet_expr.pet_get_info(self:get_occ(), 1)
end

function D_pet_obj:get_max_exp()
	return pet_expr.pet_exp(self:get_level())
end

function D_pet_obj:get_current_hp()
	return self.cur_hp
end

function D_pet_obj:set_current_hp(hp)
	self.cur_hp = hp
end

function D_pet_obj:get_current_mp()
	return self.cur_mp
end

function D_pet_obj:set_current_mp(mp)
	self.cur_mp = mp
end

function D_pet_obj:get_bind()
	return self.bind
end

function D_pet_obj:set_bind(bind)
	self.bind = bind
end

function D_pet_obj:set_hp(hp)
	self.hp  = hp or pet_expr.get_hp(self:get_occ(), self:get_stemina_ex(), self:get_level()) + self:get_equip_attr(21)
end

function D_pet_obj:get_mp()
...
function D_pet_obj:get_name()
	return self.name
end

function D_pet_obj:set_name(name)
	self.name = name
end

function D_pet_obj:get_type()
	return self.type
end

function D_pet_obj:get_exp()
	return self.exp
end

function D_pet_obj:set_exp(exp)
	self.exp = exp
end

function D_pet_obj:get_pet_id()
	return self.pet_id
end

function D_pet_obj:del_hp(hp)
	self.hp = self.hp - hp
	if self.hp < 0 then
		self.hp = 0
	end
...
function D_pet_obj:get_max_hp()
	return math.floor(pet_expr.get_hp(self:get_occ(), self:get_stemina_ex(), self:get_level()) + self:get_equip_attr(21))
end

function D_pet_obj:get_hp()
	return self.hp
end


--宠物
function D_pet_obj:update_attr(attr_list)

end

--技能
function D_pet_obj:set_skill_con(skill_con)
	self.skill_con = skill_con
end

function D_pet_obj:get_skill_con()
	return self.skill_con
end

--属性
function D_pet_obj:set_base_attr(base_attr)
	self.base_attr = base_attr
end

function D_pet_obj:get_base_attr()
	return self.base_attr
end

--效果
function D_pet_obj:set_impact_con(impact_con)
	self.impact_con = impact_con
end

function D_pet_obj:get_impact_con()
	return self.impact_con
end

--背包
function D_pet_obj:set_equip_con(equip_con)
	self.equip_con = equip_con
end

function D_pet_obj:get_equip_con()
	return self.equip_con
end

--策略
function D_pet_obj:set_strategy_obj(strategy_obj)
	self.strategy_obj = strategy_obj
end

function D_pet_obj:get_strategy_obj()
	return self.strategy_obj
end

function D_pet_obj:init_load()
	self:load_skill_con()
	--self:load_impact_con()
	self:load_equip_con()
end

function D_pet_obj:load_skill_con()
	if self.skill_con == nil then
		self.skill_con = D_skill_container(self.pet_id,self.owner_id)
		self.skill_con:load()
...
function D_pet_obj:load_impact_con()
...
function D_pet_obj:load_equip_con()
	if self.equip_con == nil then
		self.equip_con = D_pet_bag(self.pet_id, self.owner_id)
		self.equip_con:load()
	end
end

function D_pet_obj:load_base_attr(init_strengh, init_intelligence, init_stemina, init_dexterity)
	--分配四大属性
...
function D_pet_obj:get_level()
	return self.level
end

function D_pet_obj:set_level(level)
	self.level = level
end

function D_pet_obj:get_pullulate()
	return self.pullulate
end

function D_pet_obj:set_pullulate(pullulate)
	self.pullulate = pullulate
end

function D_pet_obj:get_fighting()
	return self.fighting
end

function D_pet_obj:set_fighting(fighting)
	self.fighting = fighting
end

function D_pet_obj:get_occ()
	return self.occ
end

function D_pet_obj:set_occ(occ)
	self.occ = occ
end

function D_pet_obj:get_equip_attr(index)
	local attr = self.equip_con:get_attr()
	return attr[index] or 0
end

function D_pet_obj:get_skill_attr(index)
	local attr = self.skill_con:get_passive_attr()
	return attr[index] or {}
end

function D_pet_obj:get_all_attr(index)
...
--12雷攻
function D_pet_obj:get_fire_ak()
...
--16雷抗
function D_pet_obj:get_fire_de()
...
--13毒攻
function D_pet_obj:get_poison_ak()
...
--15毒抗
function D_pet_obj:get_poison_de()
...
--物理防御
function D_pet_obj:get_s_defense()
...
--魔法防御
function D_pet_obj:get_m_defense()
...
--根骨
function D_pet_obj:get_strengh_t()
	return self:get_strengh_ex()
end

--悟性
function D_pet_obj:get_intelligence_t()
	return self:get_intelligence_ex()
end

--体魄
function D_pet_obj:get_stemina_t()
	return self:get_stemina_ex()
end

--身法
function D_pet_obj:get_dexterity_t()
	return self:get_dexterity_ex()
end

--最小,最大物攻
function D_pet_obj:get_s_attack_t()
	local min, max = 0.5 * self:get_s_attack(), self:get_s_attack()
	return math.floor(min), math.floor(max)
end

--最小,最大法攻
function D_pet_obj:get_m_attack_t()
	local min, max = 0.5 * self:get_m_attack(), self:get_m_attack()
	return math.floor(min), math.floor(max)
end

--物防
function D_pet_obj:get_s_defense_t()
	return self:get_s_defense()
end

--魔防
function D_pet_obj:get_m_defense_t()
	return self:get_m_defense()
end

--雷防
function D_pet_obj:get_fire_de_t()
	return self:get_fire_de()
end

--冰防
function D_pet_obj:get_ice_de_t()
	return self:get_ice_de()
end

--毒防
function D_pet_obj:get_poison_de_t()
	return self:get_poison_de()
end

--雷攻
function D_pet_obj:get_fire_ak_t()
	return self:get_fire_ak()
end

--冰攻
function D_pet_obj:get_ice_ak_t()
	return self:get_ice_ak()
end

--毒攻
function D_pet_obj:get_poison_ak_t()
	return self:get_poison_ak()
end

--闪避
function D_pet_obj:get_dodge_t()
	return self:get_dodge()
end

--暴击
function D_pet_obj:get_critical_t()
	return math.floor(self:get_critical())
end

--暴击效果
function D_pet_obj:get_critical_ef_t()
	return math.floor(self:get_critical_ef())
end

--命中
function D_pet_obj:get_point_t()
	return self:get_point()
end


function D_pet_obj:get_all_att_ex()
	local ret = {}
	ret[1] = self:get_name()
...
function D_pet_obj:get_net_attr_info()
	local ret = {}
...
function D_pet_obj:get_attribute()
	local ret = {}
...
function D_pet_obj:get_pet_info()
	local s_pkt = {}
	...
	for k,v in ipairs(effective_skill) do
		for k1,v1 in ipairs(skill_list[k]) do
			s_pkt.list[3][count] = v1 or 0
			count = count + 1
		end

		s_pkt.list[3][count] = v or 0
		count = count + 1
	end
	s_pkt.list[4] = {}
