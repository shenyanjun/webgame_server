...
D_pet_bag = oo.class(nil, "D_pet_bag")

function D_pet_bag:__init(pet_id, owner_id)
	self.pet_id = pet_id
...
function D_pet_bag:set_slot_list(slot,uuid,number,item)
	self.slot_list[slot] = {}
...
function D_pet_bag:get_slot_list(slot)
	return self.slot_list[slot]
end

function D_pet_bag:set_skill_attr_list()
	local ret = {}
...
function D_pet_bag:get_skill_list()
	return self.skill_list
end

function D_pet_bag:get_attr()
	return self.attr
end

function D_pet_bag:clear()
	self.slot_list = {}
	--装备属性总和
	self.attr ={}
...
--玩家在线的情况下战斗的时候同步数据
function D_pet_bag:update_bag(item_l)
	self:clear()
...
--获取背包信息
function D_pet_bag:net_get_bag_info()
	local info = {}
	local item_info
