...
D_skill_container = oo.class(nil, "D_skill_container")

function D_skill_container:__init(obj_id, owner_id)
	self.obj_id = obj_id
...
function D_skill_container:get_effective_list()
	return self.effective_skill
end

function D_skill_container:get_skill_list()
	return self.skill_list
end



function D_skill_container:init_base_skill(occ)
...
function D_skill_container:get_last_effective_skill_id(skill_id)
	if skill_id == nil then return end

	local container = g_pet_adventure_mgr:get_container(self.owner_id)
...
--获取被动技能的所有属性
function D_skill_container:set_passive_attr()
	self.passive_attr = {}
...
function D_skill_container:get_passive_attr()
	return self.passive_attr
end


function D_skill_container:use(pet_s,pet_d)
...
--统一减少cd时间
function D_skill_container:sub_cd()
	--for k, v in pairs(self.effective_skill or{}) do
		--local cd = v:get_cd()
		--if cd ~= 0 then
			--v:set_cd(cd-1)
...
--阵法闯关
function D_skill_container:use_ex(pet_s,strategy_con_s, strategy_con_d, index)
	local skill_id = self:get_effective_skill_id()
	local info = nil
	local dead_info = nil
	local addition_hp = 0
...
--统一减少cd时间
function D_skill_container:sub_cd_ex()
	for k, v in pairs(self.skill_sort) do
		if v[2] > 0 then
			v[2] = v[2] -2
			if v[2] < 0 then
...
--初始化技能cd时间
function D_skill_container:sub_all_cd()
	self.skill_sort = {}
	for k, v in pairs(self.effective_skill or{}) do
		local skill_obj = g_skill_mgr:get_skill(v)
...
function D_skill_container:get_effective_skill_id()
	for k,v in pairs(self.skill_sort) do
		if v[2] == 0 then
...
function D_skill_container:clear()
	--技能列表
	self.skill_list = {}
...
--玩家更新数据
function D_skill_container:update_skill(item_l)
	self:clear()
...
function D_skill_container:load()
	local dbh = f_get_db()
...
function D_skill_container:get_newest_effective_skill()
...
function D_skill_container:update_base_effective_skill()
	self.base_effective_skill = {0,0,0,0,0}
...
function D_skill_container:update_addition_effective_skill()
	self.addition_effective_skill = {0,0,0,0,0}
...
function D_skill_container:update_special_effective_skill()
	self.special_effective_skill = {0,0}
