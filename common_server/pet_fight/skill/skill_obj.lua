...
Skill_base = oo.class(nil,"Skill_base")

function Skill_base:__init(skill_id, type)
...
function Skill_base:get_skill_id()
	return self.skill_id
end

function Skill_base:get_type()
	return self.type
end

function Skill_base:get_cd()
	return self.cd
end

function Skill_base:set_cd(cd)
	if cd < 0 then
		cd = 0
	end
	self.cd = cd
...
function Skill_base:get_cmd_id()
	return self.cmd_id
end

function Skill_base:get_status()
	if self.cd <= 0 then
		return 0
	end
	return self.cd
...
function Skill_base:get_level()
	return self.skill_id % 100
end

function Skill_base:effect(obj_s,pet_con_s, pet_con_d)
	return 0
end
...
Skill_combat = oo.class(Skill_base,"Skill_combat")

function Skill_combat:__init(skill_id, type)
	Skill_base.__init(self, skill_id, type)
...
function Skill_combat:get_rank(obj_s, strategy_con_s, strategy_con_d)
	local pet_list_s = strategy_con_s:get_pet_list()
	local pet_id = obj_s:get_pet_id()
	local index = pet_list_s[pet_id]
	if index == nil then return end
	--print("=eeee333333333",j_e(pet_fight_config.pet_position[index]),index)
	...
	--print("333333333333",attack_index)
...
function Skill_combat:strategy_pet(index_list,strategy_con_d)
	local pet_list = {}
...
function Skill_combat:effect(obj_s,strategy_con_s, strategy_con_d)
	local pet_list = self:get_rank(obj_s,strategy_con_s, strategy_con_d)
	local info = {}
	local dead_info = {}
...
--ak:技能使用者的攻击力
--dg_type:技能伤害类型
function Skill_combat:make_hp_pkt(obj_s, obj_d, ak, dg_type)
	...
	--else
		ak = math.floor(ak)
		new_pkt[3],new_pkt[1] = damage_pet_expr(obj_s, obj_d, ak, self:get_level(), self:get_cmd_id()) * 0.5
	--end
	...
	if cmd_id == 2250700 or cmd_id == 2250800 then
		local enrich_ratio = pet_skill_config.skill_param[self.cmd_id][self:get_level()][8]
...
function Skill_combat:send_syn()
end
...
Skill_passive = oo.class(Skill_base, "Skill_passive")

function Skill_passive:__init(skill_id, type)
	Skill_base.__init(self, skill_id, type)
