...
module("vip.vip_mall_loader",package.seeall)
VipMallTable = {}


function init()
	local filename = CONFIG_DIR.."xml/mall/mall_vip.xml"
...
function handle_page(t_node)
	...
	VipMallTable[number] = {}
	VipMallTable[number].item_list ={}
	for key,value in pairs(t_node.item_list) do
		VipMallTable[number].item_list[value.item_id] = {}
		VipMallTable[number].item_list[value.item_id].id = tonumber(value.item_id)
		VipMallTable[number].item_list[value.item_id].name = value.name
		VipMallTable[number].item_list[value.item_id].price = value.price
		VipMallTable[number].item_list[value.item_id].currency = value.currency
	end
