...
function(conn, pkt)
	...
	if obj ~= nil then
		...
		local count = obj:get_copy_con():get_count_copy(obj:get_map_id())
		local new_pkt = {}
...
Clt_commands[1][CMD_MAP_ADDICTION_LOGIN_C] =
function(conn, pkt)
	local obj = g_obj_mgr:get_obj(conn.char_id)
	if obj ~= nil then
		obj:send_addiction_status()    --发送沉迷标志
	end
end

--防沉迷验证
Clt_commands[1][CMD_MAP_ADDICTION_CHECK_C] =
function(conn, pkt)
	if pkt == nil or pkt.name == nil or pkt.code == nil or pkt.flag == nil then return end
	
	local obj = g_obj_mgr:get_obj(conn.char_id)
	if obj ~= nil then
		obj:close_addiction(pkt.name, pkt.code, pkt.flag)
	end
end

--获取状态
Clt_commands[1][CMD_MAP_ADDICTION_GET_C] =
function(conn, pkt)
	local obj = g_obj_mgr:get_obj(conn.char_id)
	if obj ~= nil then
		obj:send_addiction_status()
	end
end
