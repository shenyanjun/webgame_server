...
--帮派任务外包类
local Quest_wrapper_random = oo.class(Quest_wrapper_base, "Quest_wrapper_random")

function Quest_wrapper_random:__init(meta, core)
	Quest_wrapper_base.__init(self, meta, core)
	self.char_id = 0
end

function Quest_wrapper_random:can_accept(char_id)
...
function Quest_wrapper_random:on_accept(char_id)
...
function Quest_wrapper_random:on_complete(char_id, select_list)	
	local player = g_obj_mgr:get_obj(char_id)
	if not player then return false end
	...
	local ret, list = self.core:on_complete(char_id, select_list,step_award)
...
--从数据库构造
function Quest_wrapper_random:construct(con)
	Quest_wrapper_base.construct(self,con)
	self.char_id = con.char_id
end

function Quest_wrapper_random:serialize_to_net()
	...
	if player then
		...
		if loop_faction_mission then
			result.base[4] = g_mission_mgr:get_bonus(loop_faction_mission.step, loop_faction_mission.count)
...
Mission_mgr.register_wrapper(MISSION_TYPE_RANDOM, Quest_wrapper_random)