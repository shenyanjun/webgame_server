...
--单人爬塔副本
module("scene_ex.config.tower_ex_loader", package.seeall)
...
local function parse_boss_attr7(node)
	local t = {}
	for _, child in pairs(node) do
		if "Attr" == child.tag then
			table.insert(t, {tonumber(child.attr["att"]), tonumber(child.attr["def"])} )
		end
	end
	return t
end
...
local function parse_monster_ex3(node)
	...
	for _, child in pairs(node) do
		...
		elseif "Summon" == child.tag then
			monster_ex.summon = parse_monster_ex1_item(child)
		elseif "MonsterPos" == child.tag then
			monster_ex.monster_pos = parse_monster_pos3(child)
		end
	end
	return monster_ex
end
...
local function parse_monster_ex6(node)
	...
	for _, child in pairs(node) do
		...
		elseif "Summon" == child.tag then
			monster_ex.summon = parse_monster_ex1_item(child)
		elseif "SummonPos" == child.tag then
			monster_ex.summon_pos = parse_monster_pos(child)
		end
	end
	return monster_ex
end

local function parse_monster_ex7(node)
	local monster_ex = {}
	for _, child in pairs(node) do
		if "NextLayer" == child.tag then
			monster_ex.next_layer = {tonumber(child.attr["id"]), tonumber(child.attr["area"]), tonumber(child.attr["carry_id"])}
		...
		elseif "SummonPos" == child.tag then
			monster_ex.summon_pos = parse_monster_pos(child)
		elseif "BossAttr" == child.tag then
			monster_ex.boss_attr = parse_boss_attr7(child)
		end
	end
	return monster_ex
end

local function parse_monster_ex8(node)
	return parse_monster_ex6(node)
end
...
parse_config(CONFIG_DIR .. "xml/tower/tower_ex.xml")