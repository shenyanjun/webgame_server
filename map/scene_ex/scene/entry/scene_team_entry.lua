Scene_team_entry = oo.class(Scene_copy, "Scene_team_entry")

function Scene_team_entry:__init(map_id, prototype, status)
	Scene_copy.__init(self, map_id, status)
	self.prototype = prototype
end

--副本出口
function Scene_team_entry:get_home_carry(obj)
...
--获取副本实例ID
function Scene_team_entry:get_instance_id(obj)
...
--副本创建权限检查
function Scene_team_entry:check_create_access(obj)
	...
	local human_limit = limit_config.human
	if human_limit then
	...
	local cycle_limit = limit_config.cycle
	if cycle_limit then
...
--副本进入权限检查
function Scene_team_entry:check_entry_access(obj)
	local config = g_all_scene_config[self.id]
	if not config then
		return SCENE_ERROR.E_SCENE_CLOSE, nil
	end
...
--创建副本实例
function Scene_team_entry:create_instance(instance_id, obj)
	return self.prototype(self.id, instance_id, self.map_obj:clone(self.id))
end

function Scene_team_entry:instance()
