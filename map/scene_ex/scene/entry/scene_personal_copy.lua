...
-- 个本副本
Scene_personal_copy = oo.class(Scene_copy, "Scene_personal_copy")

function Scene_personal_copy:__init(map_id, prototype, status)
	Scene_copy.__init(self, map_id, status)
	self.prototype = prototype
end

function Scene_personal_copy:get_self_config()
	return copy_config.config_list.value[self.id]
end

function Scene_personal_copy:get_self_limit_config()
	return copy_config.config_list.value[self.id]
end

--副本出口
function Scene_personal_copy:get_home_carry(obj)
...
--获取副本实例ID
function Scene_personal_copy:get_instance_id(obj)
...
--副本创建权限检查
function Scene_personal_copy:check_create_access(obj)
	--print("Scene_personal_copy:check_create_access()", obj:get_id())
...
--副本进入权限检查
function Scene_personal_copy:check_entry_access(obj)
	local config = self:get_self_limit_config()
	if not config then
		return SCENE_ERROR.E_SCENE_CLOSE, nil
	end
...
--创建副本实例
function Scene_personal_copy:create_instance(instance_id, obj)
	return self.prototype(self.id, instance_id, self.map_obj:clone(self.id))
end

function Scene_personal_copy:instance()
