local sheep_config = require("scene_ex.config.sheep_config_loader")


Scene_sheep_entry = oo.class(Scene_copy, "Scene_sheep_entry")


function Scene_sheep_entry:__init(map_id)
...
--副本出口
function Scene_sheep_entry:get_home_carry(obj)
	local config = sheep_config.config[self.id]
...
--获取副本实例ID
function Scene_sheep_entry:get_instance_id(obj)
...
function Scene_sheep_entry:obj_get_copy_id(obj_id)
	local instance_id = self.obj_instance[obj_id]
	return self.copy_index[instance_id]
end

--副本创建权限检查
function Scene_sheep_entry:check_create_access(obj)
	local target_config = g_scene_config_mgr:get_config(self.id)
	
	if not target_config then
		print("Scene_sheep_entry:check_create_access(obj)")
...
--副本进入权限检查
function Scene_sheep_entry:check_entry_access(obj)
	local target_config = g_scene_config_mgr:get_config(self.id)
	if not target_config then
		print("Scene_sheep_entry:check_entry_access(obj)")
		return SCENE_ERROR.E_SCENE_CLOSE
	end
...
--创建副本实例
function Scene_sheep_entry:create_instance(instance_id, obj)	
...
function Scene_sheep_entry:unregister_instance(instance_id, args)
	local instance = self.instance_list[instance_id]
	if instance then
...
function Scene_sheep_entry:instance()
	self.map_obj = g_scene_config_mgr:load_map(self.id)
end

function Scene_sheep_entry:pri_get_status_info()
	local copy_info = {}
	...
	return sheep_config.config.limit.count, copy_info
end

-----------------------------------------------场景入口----------------------------------------------

function Scene_sheep_entry:login_scene(obj, pos)
...
--//选择副本加多个参数
function Scene_sheep_entry:carry_scene(obj, pos)
	if self.status == SCENE_STATUS.CLOSE then
		print("Scene_sheep_entry:carry_scene(obj, pos)")
...
function Scene_sheep_entry:on_timer(tm)
...
function Scene_sheep_entry:load_today_event(today)
	local config = sheep_config.config[self.id]
...
function Scene_sheep_entry:open_event(args)
